...
interface NavItem {
  ...
  route: string; // router path this item links to
  // SVG path data for a 24x24 filled icon (avoids an icon-font dependency).
  iconPath: string;
}

// Collapsible side navigation with Arabic labels.
// Icons are inline SVG so the skeleton has no external icon dependency;
// swap to <mat-icon> once Angular Material is installed.
@Component({
...
export class SidebarComponent {
  // Collapsed (icon-only) state passed down from the layout.
  readonly collapsed = input<boolean>(false);

  // Navigation items — labels in Arabic, routes match feature paths.
